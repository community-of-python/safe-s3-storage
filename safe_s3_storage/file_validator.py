...
class FileValidator:
    ...
    def _validate_file_size(self, *, file_name: str, file_content: bytes, mime_type: str) -> int:
        ...
        if content_size > max_size:
            raise TooLargeFileError(file_name=file_name, content_type=mime_type, max_size=max_size)
        return content_size
    ...
    def _convert_image(self, validated_file: ValidatedFile) -> ValidatedFile:
        ...
        try:
            ...
            raise FailedToConvertImageError(
                file_name=validated_file.file_name, content_type=validated_file.mime_type
            ) from pyvips_error
